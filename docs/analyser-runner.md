...
4. `services/music-analyser/modal_app.py` downloads the signed audio into temporary
...
`services/music-analyser/requirements.txt`; CI uses the reviewed Python 3.11.15
...
Run the analyser unit tests from `services/music-analyser`:
...
`services/music-analyser/evals/jamendo_fixtures.json`. The reviewed musical baseline
is `services/music-analyser/evals/baseline_v1.json`.
...
```bash
python benchmark.py evals/audio/jamendo_1930820.mp3 --repeat 2
```
