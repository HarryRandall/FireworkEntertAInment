...
Add reusable reference effects in `lib/fireworks/effect-catalogue.ts`, then run `node supabase/catalogue/generate-firework-catalogue-migration.mjs` from the repository root. The generated migration upserts `firework_effects` and `fireworks`; it should not reintroduce legacy `effect_specs`, `product_shots`, or `fib-*` seed rows.
