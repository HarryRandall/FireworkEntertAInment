---
notion-url: https://www.notion.so/Day-Overview-21-04-34ccd8a5bf0881b395c3cd41a62841cf
title: "Day Overview (21/04)"
from_notion: https://www.notion.so/Day-Overview-21-04-34ccd8a5bf0881b395c3cd41a62841cf
author: From Notion
last_edited_time: '2026-04-24T08:47:00.000Z'
---
> **Scrum 1 (Sprint 2)** · 21 April 2026
  Monday stand-up disrupted: Harry unwell, Liam in Sydney; Harrison & Fang attended the Sunday/Monday Robert sync (captured in [Stakeholder Meeting 4 — 20 April 2026 (interim sync)](https://www.notion.so/34ccd8a5bf088143885cc94ca0c51bf3)). Async progress continued through the week toward the next stakeholder checkpoint (**target 4 May 2026**).


---


## What We've Done

- **Stakeholder**: Completed interim **Meeting 4** debrief with Robert (Harrison + Fang) — see linked page for decisions on MVP scope (Finale 3D-first), renderer expectations, MIR priority, and **4 May** follow-up.
- **Harry**: **Notion / sprint documentation** reformat; **platform** work at home — auth, API endpoints, and a **basic UI mockup** direction (ongoing; [FIR-51](https://linear.app/fireworkentertainment/issue/FIR-51/feature-supabase-auth-catalogue-apis-and-sprint-2-ui-mock)).
- **Liam**: **Supabase / database** work from Sydney; **documenting database helpers** and catalogue integration patterns (ongoing; overlaps [FIR-50](https://linear.app/fireworkentertainment/issue/FIR-50/catalogue-config-documentation), [FIR-45](https://linear.app/fireworkentertainment/issue/FIR-45/update-supabase-with-new-sample-database)).
- **Fang**: Evolved `prototypes/audio-analyser` into a **preprocessing module** for future agent/LLM flows — added `agent.md` + `llm-harness.md`, expanded **cue generation** and outputs in `showcrafter.py`, improved **CLI** (`--personality`), richer **Markdown report** + structured **JSON** for downstream validators/harnesses ([FIR-24](https://linear.app/fireworkentertainment/issue/FIR-24/spike-evaluate-and-select-core-mir-librariesapis), [FIR-39](https://linear.app/fireworkentertainment/issue/FIR-39/enhance-audio-analysis-granularity-and-model-generalization)).
- **Harrison**: **Godot visualiser spike** — 2D particle system approximating **two firework effect families**; exploring **VDL-driven authoring**, likely needs alignment to the **Finale 3D CSV/format** Robert standardized earlier ([FIR-28](https://linear.app/fireworkentertainment/issue/FIR-28/write-research-methods-for-firework-simulation)).
- **Process**: Recorded sprint progress and branch hygiene requests in Linear ([FIR-48](https://linear.app/fireworkentertainment/issue/FIR-48/could-you-please-help-me-delete-the-following-branches-they-are-no) done; [FIR-49](https://linear.app/fireworkentertainment/issue/FIR-49/can-you-open-the-failed-vercel-deployment-for-commit-438d9df-and-send) tracks a failed Vercel deploy investigation).

---


## What We Plan to Do (before next week)

- **Harry**: Finish **auth + core API routes** enough for a thin demo; continue **Notion** hygiene; hook UI mock to live data where possible ([FIR-51](https://linear.app/fireworkentertainment/issue/FIR-51/feature-supabase-auth-catalogue-apis-and-sprint-2-ui-mock)).
- **Liam**: Land **Supabase schema + import** updates for Robert’s latest catalogue; publish **helper/docs** for DB access; confirm **branch naming** vs. Issue & Merging Guide ([FIR-45](https://linear.app/fireworkentertainment/issue/FIR-45/update-supabase-with-new-sample-database), [FIR-25](https://linear.app/fireworkentertainment/issue/FIR-25/evaluate-database-options-for-backend-storage), [FIR-50](https://linear.app/fireworkentertainment/issue/FIR-50/catalogue-config-documentation)).
- **Fang**: Harden MIR **JSON/Markdown contracts**, wire selected libraries per spike outcomes, prep **demo audio** for Robert review ([FIR-24](https://linear.app/fireworkentertainment/issue/FIR-24/spike-evaluate-and-select-core-mir-librariesapis), [FIR-39](https://linear.app/fireworkentertainment/issue/FIR-39/enhance-audio-analysis-granularity-and-model-generalization)).
- **Harrison**: Decide **Godot vs web** presentation for prototype; start mapping **VDL / CSV fields** into sim parameters; time-permitting, stub **Finale export** experiments with Liam ([FIR-28](https://linear.app/fireworkentertainment/issue/FIR-28/write-research-methods-for-firework-simulation), [FIR-35](https://linear.app/fireworkentertainment/issue/FIR-35/implement-industry-and-generic-export-format-support-ignite-csv)).
- **Team**: Confirm **4 May** stakeholder session details; **TBD** — [placeholder] final agenda & demo checklist once Harry/Liam back in sync.

---


## Reflections


### Harry


<details>
<summary>What You’ve Done</summary>

  *To be completed.*

</details>


<details>
<summary>Roadblocks</summary>

  *To be completed.*

</details>


<details>
<summary>What You’re Gonna Do</summary>

  *To be completed.*

</details>


### Liam


<details>
<summary>What You’ve Done</summary>

  *To be completed.*

</details>


<details>
<summary>Roadblocks</summary>

  *To be completed.*

</details>


<details>
<summary>What You’re Gonna Do</summary>

  *To be completed.*

</details>


### Fang


<details>
<summary>What You’ve Done</summary>

  *To be completed.*

</details>


<details>
<summary>Roadblocks</summary>

  *To be completed.*

</details>


<details>
<summary>What You’re Gonna Do</summary>

  *To be completed.*

</details>


### Harrison


<details>
<summary>What You’ve Done</summary>

  *To be completed.*

</details>


<details>
<summary>Roadblocks</summary>

  *To be completed.*

</details>


<details>
<summary>What You’re Gonna Do</summary>

  *To be completed.*

</details>

