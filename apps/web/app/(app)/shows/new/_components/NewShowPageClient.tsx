...
import { StepPanel } from '@/app/(app)/shows/new/_components/StepPanel';
import {
  AUDIO_BUCKET,
  BUDGET_TIERS,
  MAX_AUDIO_BYTES,
  ...
  STEPS,
  WIDTH_PRESETS,
} from '@/app/(app)/shows/new/constants';
import type {
  AudioUploadState,
...
type NewShowPageClientProps = {
  initialGenerationPresentation: ShowGenerationPresentation | null;
  initialGenerationPresentationError: string | null;
};
...
const GENERATION_PRESENTATION_TIMEOUT_MS = 12_000;

async function loadGenerationPresentation() {
    ...
    return await Promise.race([
      ...
      new Promise<never>((_, reject) => {
        timeoutId = setTimeout(
          () => reject(new Error('Generation options request timed out.')),
          GENERATION_PRESENTATION_TIMEOUT_MS,
        );
      }),
    ]);
  } finally {
        ...
        if (source) {
          return {
            ...
            uploadedAudio: {
              ...
              reusedAnalysis: audio.reusedAnalysis === true,
              source,
            },
          };
        }
...
async function cleanupUnusedMusicAnalysis(uploaded: UploadedAudio): Promise<void> {
  if (uploaded.reusedAnalysis) return;
...
export default function NewShowPageClient({
  initialGenerationPresentation,
  initialGenerationPresentationError,
}: NewShowPageClientProps) {
  ...
  const [generationPresentationRequest, setGenerationPresentationRequest] = useState<number | null>(
    null,
  );
  ...
  useEffect(() => {
    if (generationPresentationRequest === null) return;
    let active = true;
    setGenerationPresentation(null);
    setGenerationPresentationError(null);
    void loadGenerationPresentation()
      .then((result) => {
        if (!active) return;
        if (result.ok) {
  ...
  const retryGenerationPresentation = () => {
    setGenerationPresentationRequest((request) => (request ?? 0) + 1);
  };
