/** Static-analysis "grep the source" test guarding the streaming-cache cleanup invariants. */
...
test('package and root README expose repository quality commands', () => {
  const pkg = JSON.parse(read('package.json'));
  const readme = readFileSync(join(repoRoot, '../../README.md'), 'utf8');
  assert.equal(pkg.type, 'module');
  assert.equal(pkg.scripts.typecheck, 'tsc --noEmit');
  assert.match(pkg.scripts.check, /pnpm lint/);
  assert.match(pkg.scripts.check, /pnpm typecheck/);
  assert.match(pkg.scripts.check, /pnpm test/);
  assert.match(pkg.scripts.check, /pnpm build/);
