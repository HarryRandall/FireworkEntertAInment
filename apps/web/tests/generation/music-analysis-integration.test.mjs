/** Static-analysis "grep the source" test guarding the music-analysis (analyser) integration invariants. */
...
test('Modal audio downloads are bounded and cannot redirect across hosts', () => {
  const modalApp = readFileSync(join(root, '../../services/music-analyser/modal_app.py'), 'utf8');
  const audioDownload = readFileSync(
    join(root, '../../services/music-analyser/audio_download.py'),
    'utf8',
  );
