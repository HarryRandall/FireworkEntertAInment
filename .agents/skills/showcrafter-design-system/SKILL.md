...
- Build UI in this order: `platform/app/components/ui` primitives first,
  domain-specific components second, route-level composition last.
- New app-level UI primitives and shared variants belong in
  `platform/app/components/ui`. Generated Radix/shadcn primitives live in
  `platform/components/ui`.
- Prefer imports from `@/app/components/ui` in new app code. Use
  `@/components/ui/...` only when extending the generated Radix/shadcn layer.
