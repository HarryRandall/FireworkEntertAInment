...
test('agent guidance and pinned repository skills stay in sync', () => {
  assert.equal(readRepo('AGENTS.md'), readRepo('CLAUDE.md'));

  const manifest = readRepo('.agents/skills/README.md');
  const skills = [
  ...
  for (const skill of skills) {
    assert.equal(existsSync(join(repoRoot, '.agents', 'skills', skill, 'SKILL.md')), true);
    assert.equal(manifest.includes('| `' + skill + '` |'), true);
  }

  assert.equal(
    existsSync(join(repoRoot, '.agents', 'skills', 'showcrafter-design-system', 'SKILL.md')),
    false,
  );
  assert.equal(
    existsSync(join(repoRoot, '.claude', 'skills', 'showcrafter-design-system', 'SKILL.md')),
    false,
  );
});
