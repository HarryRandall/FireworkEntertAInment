...
test('homepage proof uses capabilities and documented stakeholders', () => {
  ...
  const socialProof = read('app/components/marketing/SocialProof.tsx');
  const showcase = read('app/components/marketing/Showcase.tsx');
  const steps = read('app/components/marketing/Steps.tsx');
  const testimonials = read('app/components/marketing/Testimonials.tsx');
  ...
  assert.equal(existsSync(join(root, 'app/components/marketing/VendorBand.tsx')), false);
  assert.equal(existsSync(join(root, 'app/components/marketing/landing/Mockups.tsx')), false);
