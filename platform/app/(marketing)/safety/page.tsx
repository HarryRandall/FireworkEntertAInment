...
const REGIONS = [
  { state: "Queensland", auth: "Resources Safety & Health Queensland", note: "Consumer fireworks are restricted — only Type 4 sparklers and party poppers are permitted without a licence." },
  { state: "New South Wales", auth: "SafeWork NSW", note: "Most consumer fireworks require a single-use pyrotechnician's licence (excluding the Northern Territory border zone)." },
  { state: "Victoria", auth: "WorkSafe Victoria", note: "Discharging fireworks without a licence is prohibited. ShowCrafter shows can be designed but must be fired by a licensed operator." },
  { state: "Northern Territory", auth: "WorkSafe NT", note: "The only Australian jurisdiction with a public consumer fireworks period — Territory Day, 1 July." },
];
...
export default function SafetyPage() {
  return (
    <>
      ...
      <section className="border-b border-outline-variant/15 bg-surface-container-lowest py-12">
        <Container>
          <div className="flex flex-col items-start gap-4 rounded-2xl border border-[color:var(--color-danger)]/30 bg-[color-mix(in_srgb,var(--color-danger)_10%,transparent)] p-6 md:flex-row md:items-center">
            ...
            <div>
              ...
              <p className="mt-1 text-sm leading-relaxed text-on-surface-variant">
                Most Australian states require a licensed pyrotechnician to discharge
                consumer fireworks. ShowCrafter helps you design — you are responsible
                for legal compliance when you fire.
              </p>
            </div>
          </div>
        </Container>
        ...
        <Container>
          <div className="mx-auto max-w-4xl">
            ...
            <div className="mt-12 space-y-3">
              {REGIONS.map((region) => (
                <Card key={region.state} radius="md" className="flex flex-col gap-3 p-6 md:flex-row md:items-center">
                  <div className="md:w-48">
                    <h3 className="text-base font-bold text-on-surface">{region.state}</h3>
            ...
            <p className="mt-8 text-xs text-on-surface-variant">
              The information above is a general summary. Always confirm current
              regulations with your state's authority before firing any pyrotechnic.
            </p>
          </div>
        </Container>
      </section>
