...
const STEPS = [
  {
    ...
    eyebrow: 'Step 01',
    title: 'Pick a song',
    body: 'Upload an MP3 or paste a streaming link. Our audio engine analyses tempo, beats, drops and harmonic peaks — the moments that deserve a sky reaction.',
  },
  {
    icon: Sparkles,
    eyebrow: 'Step 02',
    title: 'Set a budget',
    body: "Tell us what you can spend, where you'll fire it, and how big the venue is. We size cakes, single-shots and finales accordingly — never overshooting.",
  },
  {
    icon: Wand2,
    eyebrow: 'Step 03',
    title: 'Let the AI choreograph',
    body: 'Our agent maps every cue to a real ICON Pyrotechnics product, scoring colour, height and effect against the music. You get a full timeline in seconds.',
  },
  {
    icon: ShoppingBag,
    eyebrow: 'Step 04',
    title: 'Buy the exact products',
    body: 'ShowCrafter generates a shopping list keyed to the cues. Pick them up from your local ICON stockist or order direct — every SKU is in-stock and legal.',
  },
  {
    icon: ShieldCheck,
    eyebrow: 'Step 05',
    title: 'Fire the show',
    body: 'Print the show guide. Lay out your fireworks, follow the cue numbers, and press play — the audio click track tells you exactly when to light each one.',
  },
];
            ...
            {STEPS.map((step) => {
              ...
              return (
                <Card key={step.title} radius="lg" className="p-8" hoverable>
                  <div className="bg-primary/15 text-primary mb-5 inline-flex h-12 w-12 items-center justify-center rounded-full">
      ...
      <section className="border-outline-variant/15 bg-surface-container-lowest border-t py-24">
        <Container>
          <div className="mx-auto max-w-3xl">
            ...
            <h2 className="text-on-surface mt-3 text-center text-3xl font-bold tracking-tight md:text-5xl">
              Questions, answered.
            </h2>
            <div className="mt-12 space-y-4">
              {FAQS.map((faq) => (
                <Card key={faq.q} radius="md" className="p-6">
                  ...
                  <p className="text-on-surface-variant mt-2 pl-7 text-sm leading-relaxed">
                    {faq.a}
                  </p>
                </Card>
              ))}
