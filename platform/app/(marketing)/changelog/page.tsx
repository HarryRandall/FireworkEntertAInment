            ...
            {RELEASES.map((release) => (
              <Card key={release.version} radius="lg" className="p-8">
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  <span className="text-on-surface font-mono text-sm font-bold tabular-nums">
                    {release.version}
                  </span>
                  <span className="text-on-surface-variant text-sm">·</span>
                  <Eyebrow tone="muted">{release.date}</Eyebrow>
                </div>
                <h3 className="text-on-surface mt-4 text-2xl font-bold tracking-tight">
                  {release.title}
                </h3>
                <ul className="mt-5 space-y-2.5">
                  {release.bullets.map((bullet) => (
                    <li
                      key={bullet}
                      className="text-on-surface-variant flex items-start gap-3 text-sm leading-relaxed"
                    >
