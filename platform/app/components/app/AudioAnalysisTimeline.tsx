...
function statusDescription(hasAudio: boolean, analysis: ShowAnalysisSnapshot | null): string {
  ...
  if (!analysis) return "The analysis starts automatically after the show is created.";
  ...
  return "AI-ready song context has been generated for downstream show planning.";
}
  ...
  return (
    ...
    >
      ...
      {initialAnalysis?.status === "failed" ? (
        <div className="mb-5 flex items-start gap-3 rounded-lg border border-error/35 bg-error/10 p-4 text-sm text-on-surface">
      ...
      <div className="grid gap-3 sm:grid-cols-4">
        ...
        <StatTile
          label="Completed"
          value={initialAnalysis?.completedAt ? "Yes" : "-"}
        />
      </div>

      <div className="mt-7 rounded-lg border border-outline-variant/45 bg-surface/70 p-4">
        ...
        {contextPreview ? (
          <pre className="max-h-[420px] overflow-auto whitespace-pre-wrap rounded-md bg-surface-container-low p-4 text-xs leading-relaxed text-on-surface">
            {contextPreview}
          </pre>
        ) : (
          <div className="rounded-md bg-surface-container-low p-4 text-sm text-on-surface-variant">
            {hasAudio ? "Song context will appear here when the background analysis finishes." : "No song context is available without audio."}
          </div>
        )}
      </div>
    </Card>
