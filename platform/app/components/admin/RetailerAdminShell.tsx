...
import { SIDEBAR_BRAND_BUTTON_CLASS } from '@/app/components/shell-utils';
...
function isActivePath(pathname: string | null, href: string) {
  return pathname === href || (href !== '/retailer-admin' && Boolean(pathname?.startsWith(`${href}/`)));
}
    ...
    >
      ...
      <SidebarInset>
        <header className="bg-background/95 supports-[backdrop-filter]:bg-background/85 border-border flex h-14 shrink-0 items-center gap-2 border-b px-4 backdrop-blur sm:px-6">
          <SidebarTrigger className="shrink-0 md:hidden" aria-label="Open retailer admin navigation" />
