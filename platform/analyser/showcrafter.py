...
class AnalysisResultModel(SchemaModel):
    ...
    beat_times: Annotated[list[NonNegativeFloat], Field(max_length=200_000)]
    onset_times: Annotated[list[NonNegativeFloat], Field(max_length=200_000)]
    energy_timeline: Annotated[list[EnergyPointModel], Field(max_length=50_000)]
    sections: Annotated[list[SectionModel], Field(min_length=1, max_length=1_000)]
    key_moments: Annotated[list[KeyMomentModel], Field(max_length=10_000)]
    buildups: Annotated[list[BuildupModel], Field(max_length=10_000)]
    music_profile: MusicProfileModel
    show_personality: ShowPersonalityModel
    firework_cues: Annotated[list[FireworkCueModel], Field(max_length=200_000)]
    derived: DerivedFeaturesModel
    downbeat_times: Annotated[list[NonNegativeFloat], Field(max_length=200_000)]
    ...
    def validate_times_within_duration(self):
        ...
        for index in (self.derived.quietest_section_index, self.derived.highest_energy_section_index):
            if index is not None and index >= section_count:
...
def validate_schema(model_cls: type[BaseModel], payload: dict, label: str) -> dict:
    try:
        ...
        if derived is not None:
            # These fields are required-but-nullable in the live TypeScript
            # schema. Preserve their keys while optional cue/anchor fields
            # continue to be omitted when they have no value.
            for field in (
                "finale_window",
...
def select_climax_indices(
    ...
    def section_for_time(t: float) -> dict | None:
        return get_section_at_time(t, sections)
...
def preflight_audio_duration(file_path: str) -> float | None:
    ...
    try:
        ...
        # Local contributor environments may not have ffprobe. The Modal image
        # does, and the bounded librosa decode below remains the safety net.
        return None
...
def analyse_song(
    ...
    *,
    ...
    # Probe first, then cap decoding independently. The decode reads at most
    # one sample beyond the supported duration, enough to distinguish a file
    # at the exact boundary from an overlong one without materialising hours
    # of low-bitrate audio.
...
def compute_derived_features(result: dict) -> dict:
    ...
    if last_climax_t is not None:
        ...
        if leading_buildup is not None:
            ...
            containing_section = get_section_at_time(last_climax_t, sections)
            if containing_section is not None:
                finale_start = float(containing_section["start"])
        finale_window = {
...
def build_firework_cue_summary(cues: list[dict], sections: list[dict]) -> dict:
    ...
    for section in sections:
        section_cues = [
            cue for cue in cues
            if get_section_at_time(cue["time"], sections) is section
        ]
        section_counts = {}
...
def build_llm_payload(result: dict) -> dict:
    ...
    for i, s in enumerate(sections):
        cue_counts = {}
        for c in cues:
            if get_section_at_time(c["time"], sections) is s:
...
def write_markdown(result: dict, output_path: str):
    ...
    for i, s in enumerate(r['sections']):
        ...
        section_cues = [
            c for c in r['firework_cues']
            if get_section_at_time(c['time'], r['sections']) is s
        ]
        cue_counts = {}
