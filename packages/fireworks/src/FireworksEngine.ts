...
import { FIREWORKS_ENGINE_FIXED_STEP_SECONDS } from './clock.ts';
import {
...
import { HIDDEN_PARTICLE_SHAPE, type Particle } from './Particle.ts';
...
import { replayCuesSimulationKey } from './replay-cache-key.ts';
...
import { type FireworkSceneMode, World } from './World.ts';
...
export type FireworksEngineOptions = {
  soundAssets?: import('./SoundHandler.ts').SoundAssets;
...
export class FireworksEngine {
  private readonly reportedCues = new Set<string>();
  ...
  constructor(
    ...
    private readonly options: FireworksEngineOptions = {},
  ) {
    this.scene = scene;
    this.pool = new ParticlePool(PARTICLE_CAPACITY);
    this.sound = new SoundHandler(options.soundAssets);
  ...
  private fireCue(cue: ReplayCue, audible: boolean): void {
    if (!cue.firework.renderDesign) {
      if (!this.reportedCues.has(cue.id)) {
        this.reportedCues.add(cue.id);
        this.options.onDiagnostic?.({
          path: [],
          message: 'Cue skipped because its render settings are invalid.',
          recordId: cue.firework.id,
          cueId: cue.id,
          rendererFingerprint: this.options.rendererFingerprint,
        });
      }
      return;
    }
    const baseDesign = scaleDesignForCaliber(cue.firework.renderDesign, cue.firework.caliber);
