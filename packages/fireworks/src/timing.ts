...
export function isGroundFireworkEffect(design: FireworkDesign): boolean {
  return isGroundGeometry(design.geometry);
}
...
function emittedDurationSeconds(design: FireworkDesign): number {
  return design.geometry === 'roman_candle' || design.geometry === 'fountain'
    ? groundEmissionDuration(design)
    : 0;
}
...
export function estimateFireworkLaunchTrailEndSeconds(
  ...
  const particleLife =
    (liftParticles.lifetime.baseSeconds + liftParticles.lifetime.afterglowSeconds) * lifeVariation;
...
export function estimateFireworkLaunchSmokeEndSeconds(
  ...
  const emitsDuringAscent = !isGroundFireworkEffect(design) && smoke.height > 0;
